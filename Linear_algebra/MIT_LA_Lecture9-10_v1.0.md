...
假设$$V$$是在域 $$K$$ 上的向量空间，如果 $$ \vec{v}_1,…,\vec{v}_n$$ 是$$V$$的向量，称他们为线性相关，如果从域 $$K$$ 有非全零的元素的$$a_1,a_2,…,a_n$$，使得
$$
a_1 \vec v_1+a_2 \vec v_2+...+a_n \vec v_n=\vec 0
\tag{1}
$$
如果 $$K$$ 中不存在这样的元素，那么 $$ \vec{v}_1,…,\vec{v}_n$$ 是线性无关。
...
1. 在一条直线上的向量线性相关。
2. 零向量与任何向量线性相关。
3. 二维平面内，任意三个向量线性相关；$$n$$ 维空间内，任意 $$n+1$$ 个向量线性相关。



## 性质

1. 线性无关等价于矩阵 $$A$$ 零空间只有零向量，即 $$rank(A) = n$$；线性相关等价于矩阵 $$A$$ 零空间有非零向量，即 $$rank(A) < n$$ 。

2. 向量 $$ \vec{v}_1,…,\vec{v}_n$$ 张成（span）一个空间意味着，这个空间包含这些向量的所有线性组合。
